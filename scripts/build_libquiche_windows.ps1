...
.OUTPUTS
  build/quiche/windows/<arch>/quiche.dll          (no `lib` prefix — cargo/MSVC convention)
  build/quiche/windows/<arch>/quiche.dll.lib      (import library)
...
# cargo's cdylib output on Windows MSVC: `quiche.dll` + `quiche.dll.lib`
# (no `lib` prefix — that's a Unix convention rustc only applies on non-Windows
# targets). The staticlib output is the separate `quiche.lib`.
$cargoRelease = Join-Path $QuicheDir "target/$RustTarget/release"
Copy-Item (Join-Path $cargoRelease "quiche.lib")     $OutDir -Force
Copy-Item (Join-Path $cargoRelease "quiche.dll")     $OutDir -Force
Copy-Item (Join-Path $cargoRelease "quiche.dll.lib") $OutDir -Force
Copy-Item (Join-Path $QuicheDir "quiche/include/quiche.h") (Join-Path $OutDir "include") -Force

Write-Host "Built quiche.{lib,dll,dll.lib} -> $OutDir/"
