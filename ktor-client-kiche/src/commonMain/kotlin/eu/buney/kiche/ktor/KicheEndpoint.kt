...
internal class KicheEndpoint(
    ...
    private suspend fun eventLoop() {
        ...
        try {
            while (true) {
                // Read connection state under mutex to avoid racing with closeConnectionLocked()
                val (c, socket) = mutex.withLock {
                    val c = conn ?: return
                ...
                // Get quiche timeout under mutex (reads connection state)
                val timeout = mutex.withLock { c.timeoutAsMillis().coerceIn(1, 1000) }
                ...
                mutex.withLock {
                    if (c.isClosed) return@withLock

                    if (dgram != null) {
                        val bytes = dgram.packet.readByteArray()
                        c.recv(buf = bytes, len = bytes.size, from = peer!!, to = local!!)
                    } else {
                        c.onTimeout()
                    }

                    // Drive pending body sends for all active streams
                    driveBodySendsLocked()

                    // Poll and dispatch H3 events
                    val h3 = h3Conn ?: return@withLock
                    while (true) {
                        val event = h3.poll(quicConn = c) ?: break
                        dispatchEventLocked(event, recvBodyBuf)
                    }
            ...
            // Connection closed — clean up resources and fail all pending streams inline.
            // Do NOT launch a coroutine here: the scope may already be cancelled, which
            // would prevent launch from executing and leak native resources.
     ...
     */
    private fun dispatchEventLocked(event: KicheH3Event, recvBodyBuf: ByteArray) {
        ...
        when (event.type) {
            ...
            KicheH3EventType.Data -> {
                while (true) {
                    val n = try {
                        h3Conn!!.recvBody(quicConn = conn!!, streamId = event.streamId, buf = recvBodyBuf)
            ...
            KicheH3EventType.GoAway -> {
                // GoAway is connection-level: fail all streams with IDs >= the GoAway stream ID.
                // quiche reports the GoAway stream ID via event.streamId.
     ...
     */
    private suspend fun driveBodySendsLocked() {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                val result = trySendBodyData(
                    h3Conn!!, conn!!, stream.streamId, bodyChannel, stream.bodyReadBuf!!,
    ...
    private fun drainSendLocked() {
        ...
        while (true) {
            ...
            if (!socket.outgoing.trySend(Datagram(packet, peerAddr)).isSuccess) break
        }
    }

    /** Closes connection resources. Called under mutex. Safe to call multiple times. */
    private fun closeConnectionLocked() {
        ...
        quicConfig?.close()
        quicConfig = null
        // Socket may already be closed by close() — guard against double-close.
